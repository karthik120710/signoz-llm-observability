...
/**
 * Auto-generated BudgetAlert Routes
 *
...
import type { BudgetAlertHooks } from "./budget-alert.hooks"
...
/**
 * Set hooks for BudgetAlert routes
 */
export function setBudgetAlertHooks(h: BudgetAlertHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["ruleId", "ruleName", "alertType", "currentTokens", "currentCost", "limitTokens", "limitCost", "threshold", "message", "acknowledged", "createdAt"] as const
...
/**
 * Create BudgetAlert CRUD routes
 */
export function createBudgetAlertRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[BudgetAlert] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.budgetAlert.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "BudgetAlert not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[BudgetAlert] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.budgetAlert.create({
        data: picked.data,
      })
      ...
      console.error("[BudgetAlert] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.budgetAlert.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[BudgetAlert] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[BudgetAlert] Delete error:", error)
