...
/**
 * Auto-generated SREIncident Routes
 *
...
import type { SREIncidentHooks } from "./sreincident.hooks"
...
/**
 * Set hooks for SREIncident routes
 */
export function setSREIncidentHooks(h: SREIncidentHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["sigNozAlertId", "title", "description", "severity", "status", "service", "rootCause", "hypothesis", "resolution", "affectedTraces", "sigNozQuery", "createdAt", "updatedAt"] as const
...
/**
 * Create SREIncident CRUD routes
 */
export function createSREIncidentRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        ...
        prisma.sREIncident.count({ where }),
      ])

      return sendJson(c, { ok: true, items, total })
    } catch (error: any) {
      console.error("[SREIncident] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.sREIncident.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "SREIncident not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[SREIncident] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.sREIncident.create({
        data: picked.data,
      })
      ...
      console.error("[SREIncident] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.sREIncident.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[SREIncident] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      await prisma.sREIncident.delete({
        where: { id },
      })
      ...
      console.error("[SREIncident] Delete error:", error)
