...
/**
 * Auto-generated LLMUsage Routes
 *
...
import type { LLMUsageHooks } from "./llmusage.hooks"
...
/**
 * Set hooks for LLMUsage routes
 */
export function setLLMUsageHooks(h: LLMUsageHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["provider", "model", "inputTokens", "outputTokens", "totalTokens", "cost", "latencyMs", "status", "agentName", "sessionId", "traceId", "spanId", "errorMessage", "metadata", "createdAt"] as const
...
/**
 * Create LLMUsage CRUD routes
 */
export function createLLMUsageRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[LLMUsage] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.lLMUsage.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "LLMUsage not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[LLMUsage] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.lLMUsage.create({
        data: picked.data,
      })
      ...
      console.error("[LLMUsage] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.lLMUsage.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[LLMUsage] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[LLMUsage] Delete error:", error)
