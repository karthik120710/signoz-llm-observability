...
/**
 * Auto-generated SigNozConfig Routes
 *
...
import type { SigNozConfigHooks } from "./sig-noz-config.hooks"
...
/**
 * Set hooks for SigNozConfig routes
 */
export function setSigNozConfigHooks(h: SigNozConfigHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["instanceUrl", "apiKey", "region", "mcpEnabled", "mcpUrl", "connected", "lastSyncAt", "createdAt", "updatedAt"] as const
...
/**
 * Create SigNozConfig CRUD routes
 */
export function createSigNozConfigRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        ...
        prisma.sigNozConfig.count({ where }),
      ])

      return sendJson(c, { ok: true, items, total })
    } catch (error: any) {
      console.error("[SigNozConfig] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.sigNozConfig.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "SigNozConfig not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[SigNozConfig] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.sigNozConfig.create({
        data: picked.data,
      })
      ...
      console.error("[SigNozConfig] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.sigNozConfig.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[SigNozConfig] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      await prisma.sigNozConfig.delete({
        where: { id },
      })
      ...
      console.error("[SigNozConfig] Delete error:", error)
