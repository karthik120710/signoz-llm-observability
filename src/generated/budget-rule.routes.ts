...
/**
 * Auto-generated BudgetRule Routes
 *
...
import type { BudgetRuleHooks } from "./budget-rule.hooks"
...
/**
 * Set hooks for BudgetRule routes
 */
export function setBudgetRuleHooks(h: BudgetRuleHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["name", "scope", "scopeValue", "limitType", "maxTokens", "maxCost", "alertThreshold", "action", "enabled", "createdAt", "updatedAt"] as const
...
/**
 * Create BudgetRule CRUD routes
 */
export function createBudgetRuleRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        ...
        prisma.budgetRule.count({ where }),
      ])

      return sendJson(c, { ok: true, items, total })
    } catch (error: any) {
      console.error("[BudgetRule] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.budgetRule.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "BudgetRule not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[BudgetRule] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.budgetRule.create({
        data: picked.data,
      })
      ...
      console.error("[BudgetRule] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.budgetRule.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[BudgetRule] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      await prisma.budgetRule.delete({
        where: { id },
      })
      ...
      console.error("[BudgetRule] Delete error:", error)
